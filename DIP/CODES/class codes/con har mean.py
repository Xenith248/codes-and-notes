...
for i in range(1,l-1):
    for j in range(1,w-1):
        ...
        s_out_img[i][j]=(np.sum(s_img_seg**(sq+1)))//(np.sum(s_img_seg**sq))
        p_out_img[i][j]=(np.sum(p_img_seg**(pq+1)))//(np.sum(p_img_seg**pq))
...
ax4.imshow(s_out_img,cmap='gray')
ax5 = fig.add_subplot(2,3,5)
ax5.imshow(p_out_img,cmap='gray')
plt.show()
